...
export const dividedIntoSections = list => {
  ...
  Object.values(list).forEach(value => {
    const category = numberToLigue(parseInt(value.cislo, 10));
    const month = numberToMonth2(parseInt(parseDate(value.datum).month, 10));
...
export const findRozhodcaMatches = (refID, refs) => {
  let matchIds = [];
  const ref = _.find(refs, { id: refID });
  if (ref && ref.zapasy) {
    Object.values(ref.zapasy).forEach(value => {
